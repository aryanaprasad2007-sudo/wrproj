...
function registerTimezones(vcalendar) {
  for (const vtz of vcalendar.getAllSubcomponents('vtimezone')) {
    try {
      ...
      console.warn('[grimoire] skipped a VTIMEZONE block:', err);
    }
  }
}
...
export function parseEvents(icsText, windowStart, windowEnd, cfg = {}, source = null) {
  ...
  for (const ve of vevents) {
    ...
    try {
      ...
      console.warn('[grimoire] skipped an unparseable VEVENT:', err);
      continue;
    }
  ...
  for (const ex of exceptions) {
    ...
    if (master) {
      try {
        ...
        console.warn('[grimoire] could not attach a recurrence override:', err);
      }
    }
  ...
  for (const event of masters) {
    ...
    try {
      ...
      console.warn('[grimoire] could not expand a repeating event:', event.summary, err);
      pushSingle(event);
      continue;
    }
    ...
    while ((next = iterator.next())) {
      if (++steps > cap) {
        console.warn(`[grimoire] stopped expanding "${event.summary}" after ${cap} occurrences`);
        break;
      }
      let details;
      try {
        ...
        console.warn('[grimoire] bad occurrence skipped:', err);
        continue;
      }
...
export function parseAllFeeds(feeds, windowStart, windowEnd, cfg) {
  ...
  for (const feed of feeds) {
    ...
    try {
      ...
      console.warn(`[grimoire] could not parse "${feed.source.label}":`, err);
