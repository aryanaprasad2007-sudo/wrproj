# Forward-test weekly review — 2026-07-17

Venue: **Webull PAPER (UAT)**

## Verdict

**TOO EARLY TO JUDGE (1/30 closed trades). Keep running.**

Variance context (Monte Carlo): current losing streak 0 (normal up to 12); a window net down to $-1,030 is normal; 17.5% of 6-week windows lose money even when the system works.

| metric | actual | benchmark |
|---|---|---|
| Equity | $2,000 sim book | scored from fills/levels — the shared Webull UAT balance is not meaningful |
| Closed trades | 1 | judge at ≥30 |
| Win rate | 0.0% | ~48% |
| Profit factor | — | 1.3 |
| Trades/week | 0.6 | ~8.4 |
| Median slippage/share | $0.2000 | $0.01 assumed in backtest |

## Basket vs basket (the universe-scan question)

| basket | closed | net | PF | benchmark PF |
|---|---|---|---|---|
| current | 1 | $0 | inf | 1.3 |
| candidate | 0 | — | — | 1.4 |

## Closed round-trips

| symbol   | entry_ts            | exit_ts             |   qty |   pnl | events          |
|:---------|:--------------------|:--------------------|------:|------:|:----------------|
| NVDA     | 2026-07-14T10:00:12 | 2026-07-15T09:33:14 |     2 |   nan | ['BUY', 'STOP'] |

## Still open: NVDA

## DAILY track — iAPE-D on the control basket

Queued signals to date: 3 · fills: 2 · expired/skipped: 1 · still open: UNH, JPM

No closed daily trades yet — at ~0.4/wk that is expected for the first weeks.

*Benchmark pre-registered 2026-07-02 from the 2y variant-B backtest. Not financial advice.*
