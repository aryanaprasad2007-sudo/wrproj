---
name: rerank-top5
description: Re-judge Ari's Top 5 on The Docket right now, mid-day, without a full morning rebuild. Use when he says "re-rank my Top 5", "redo my top 5", "what should I actually focus on right now", "curate my day again", or asks Claude to reconsider what's most important today given something that's changed since the 6 AM run.
---
...
5. Re-derive `topPriorities` and `topBacklog` using the SAME judgment rubric
   ...
   (`C:\Users\aware\.claude\scheduled-tasks\daily-docket\SKILL.md`) for the
...
7. Run `py build_docket.py` (from `Daily-Docket/`), then republish to the
   SAME live artifact: `url` `https://claude.ai/code/artifact/98bafd13-a2a6-4b05-83a8-84c357fa4dc1`,
